...
### 🚀 新特性与重构
- ✨ **LangGraph 多节点流水线重构**
  - 将测试用例生成智能体从单次全套输出重构为 `Plan-and-Execute` （计划与拆解处理）分布网络
  - 新增 `plan_cases`、`generate_single_case` 节点，彻底解耦调度与编码职责
- ✨ **AST 静态语法查杀与退回防线**
  - 引入 `validate_python_code` 原生词法验证
  - 智能用例在后台接受无侵入式编译审查。若未通过，通过条件边打回并携带详细溯源报告；连续3次失败后降级封禁(`enabled=False`)，确保流程不阻断
- ✨ **高级 Prompt 思考链 (CoT) 与动态外挂**
  - 写入 Playwright 重度浓缩版 `Cheat Sheet`（语法备忘录），死限框架语法不出轨
  - 强制引入 `<thinking>` 内部消化缓冲，大模型不再瞎编直接出码，提升逻辑连贯度
- ✨ **无损数据透视补全**
  - 重梳 Context 全景透传通道：所有的中间路由(`dom_analysis`, `code_analysis`, `test_strategy`) 均直接传输给 Planner 与各个 Coder 节点，告别大模型“零视野乱猜”现象

### 🐛 问题修复
- 🐛 **底层 WebSocket 通信泛洪与日志刷屏消除**
  - 拦截流式碎片 (`level: "stream"`) 的重复冗余底层终端打印流，保卫极客终端洁净度
- 🐛 **JSON 取壳鲁棒性修复**
  - 在大纲推断节点改用高阶正则表达式提取大括号数组，无视模型的任意中文废话回答包浆
- 🐛 **前端空执行断言容改**
  - 修正了后端挂掉时前端依旧欢天喜地宣告“成功生成 0 个”的谎言报文，现在能给前端准确输送红色异常截断原因
## v0.1.11 (2026-04-15)
